...
# Aug-24 parse = pa.Parse_CL(category='ноутбук', scraper='selenium', num_outfile=0, interrupt=15).Pagination() #Следить - все ли (для rquest); selenium подрубается со второго запуска
# Aug-24 parse = pa.Parse_DNS(category='ноутбук', scraper='selenium', num_outfile=1, interrupt=5, pagination_start=23).Pagination() #Следить - все ли
...
# Aug-24 parse = pa.Parse_OZ(category='ноутбук', scraper='selenium', num_outfile=0, interrupt=5).Pagination()
# Aug-24 parse = pa.Parse_Ya(category='ноутбук', scraper='selenium', num_outfile=2, interrupt=0, user_id=1).\
#Pagination(vendors=[])
...
#   Pagination(vendors=[])
# aug-24 parse = pa.Parse_DNS(category='проектор', scraper='selenium', num_outfile=0, interrupt=5).Pagination()
...
#parse = pa.Parse_DNS(category='ибп', scraper='selenium', num_outfile=1, interrupt=5).Pagination()
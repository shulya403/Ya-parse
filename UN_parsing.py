...
#parse = pa.Parse_CL(category='ноутбук', scraper='selenium', num_outfile=0, interrupt=15).Pagination() #Следить - все ли (для rquest); selenium подрубается со второго запуска
#parse = pa.Parse_DNS(category='ноутбук', scraper='selenium', num_outfile=0, interrupt=5).Pagination() #Следить - все ли
...
#parse = pa.Parse_Ya(category='монитор', scraper='selenium', num_outfile=1, interrupt=1, user_id=1).Pagination(vendors=["Asus", "BenQ", "Dell"])
# "Asus", "BenQ", "Dell", !!!!!

# Mnt ["Acer","AOC", "Asus", "BenQ", "Dell", "HP", "Iiyama", "LG", "Philips", "Samsung", "Viewsonic", "Lenovo", "MSI", "NEC", "Xiaomi", "Gigabyte"]

#parse = pa.Parse_Ya(category='проектор', scraper='selenium', num_outfile=0, interrupt=1, user_id=1).Pagination(vendors=[])
# Prj ['Acer','BenQ','Epson','Infocus','LG','NEC','Optoma','Sony','Viewsonic','Xiaomi','Barco','Canon','Casio','Christie','Hiper ','Hitachi','JVC','Panasonic',
#             'Ricoh','Smart','Vivitek']

#parse = pa.Parse_Ya(category='ибп', scraper='selenium', num_outfile=0, interrupt=1).Pagination(vendors=[])

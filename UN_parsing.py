...
#parse = pa.Parse_Ya(category='ноутбук', scraper='selenium', num_outfile=0, interrupt=10, user_id=1).\
#    Pagination(vendors=[])
...
parse = pa.Parse_Ya(category='монитор', scraper='selenium', num_outfile=1, interrupt=2, user_id=1).\
   Pagination(vendors=["Viewsonic"])

# Mnt ["Acer","AOC", "Asus", "BenQ", "Dell", "HP", "Iiyama", "LG", "Philips", "Samsung", "Viewsonic", "Lenovo", "MSI", "NEC", "Xiaomi", "Gigabyte", "Huawei"]

#parse = pa.Parse_Ya(category='проектор', scraper='selenium', num_outfile=1, interrupt=5, user_id=1).Pagination(vendors=['BenQ', 'Epson', 'Infocus', 'LG', 'Optoma', 'Sony', 'Viewsonic', 'Xiaomi', 'Barco', 'Canon', 'Casio', 'Christie', 'Hiper', 'Hitachi', 'JVC', 'Panasonic', 'Ricoh', 'Smart', 'Vivitek'])
